...
* Transcribe a single audio file `sample.wav`  
	...
	- The options `-v`, `--ckpt_path`, `--decoder_name` are parameters for ASR models. To find out which parameters shuld be provided for corresponding ASR models, please refer to the config file `configs/model_config.yaml`.
...
- Start ASR web service on the docker container. For example, the following command line starts the Jasper model as web service
	```
	sudo docker exec -ti [your_container_name] python /workspace/siren/asr_services/server.py --port 6006 --asr_name Jasper --model_config /workspace/siren/checkpoints/quartznet15x5/quartznet15x5.yaml --ckpt_dir /workspace/siren/checkpoints/quartznet15x5
	``` 
	Remarks
...
- **Attack tasks.** Run all the attack tasks in the docker container built from the docker file `asr_services/dockerfiles/Dockerfile_core`
  * Build docker images
  	```
	docker image build -t siren:v2 - < asr_services/dockerfiles/Dockerfile_core
  	```
